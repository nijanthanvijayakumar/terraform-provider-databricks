...
func TestServiceCredentialsCornerCases(t *testing.T) {
	qa.ResourceCornerCases(t, ResourceServiceCredential())
}

func TestCreateServiceCredential(t *testing.T) {
	qa.ResourceFixture{
		...
		Resource: ResourceServiceCredential(),
		Create:   true,
		HCL: `
		name = "a"
		aws_iam_role {
...
func TestCreateIsolatedServiceCredential(t *testing.T) {
	qa.ResourceFixture{
		MockWorkspaceClientFunc: func(w *mocks.MockWorkspaceClient) {
			e := w.GetMockCredentialsAPI().EXPECT()
			e.CreateCredential(mock.Anything, catalog.CreateCredentialRequest{
